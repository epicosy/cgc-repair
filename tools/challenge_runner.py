...
def get_core_dump_regs(path, pid, log, cores_path):
    ...
    if IS_DARWIN:
        ...
        if cores_path:
            core = '{}/core.{}.{}.{}'.format(cores_path, hostname, pid, path.replace('/', '!'))
...
def clean_cores(paths, procs, cores_path):
    ...
    if IS_DARWIN:
        ...
        hostname = socket.gethostname()
        if cores_path:
            map(try_delete, ['{}/core.{}.{}.{}'.format(cores_path, hostname, proc.pid, path.replace('/', '!')) for proc, path in zip(procs, paths)])
        else:
            try_delete('core')
    elif IS_WINDOWS:
        for path, proc in zip(paths, procs):
